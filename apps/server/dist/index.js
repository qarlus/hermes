...
var RELAY_VERSION = "0.2.0";
var DEFAULT_HOST = process.env.HERMES_RELAY_HOST ?? "127.0.0.1";
var DEFAULT_PORT = Number.parseInt(process.env.HERMES_RELAY_PORT ?? "8787", 10);
var DEFAULT_DATA_PATH = resolve(process.cwd(), process.env.HERMES_RELAY_DATA ?? "./data/relay.json");
async function createHermesRelayStore(dataPath = DEFAULT_DATA_PATH) {
  return new HermesRelayStore(dataPath);
}
async function startHermesRelay(options) {
...
class HermesRelayStore {
  ...
  async connectWorkspace(input, relayUrl) {
    ...
    if (bootstrap) {
      ...
      if (!workspace) {
        ...
        return this.buildSession(store.relayId, workspace, input.deviceId, relayUrl, workspace.adminToken);
      }
  ...
  async inspectWorkspace(input, relayUrl) {
    const store = await this.load();
    ...
    await this.save(store);
    return this.buildSession(store.relayId, workspace, device.id, relayUrl, input.adminToken?.trim() === workspace.adminToken && device.role === "master" ? workspace.adminToken : null);
  }
  async approveDevice(input, relayUrl) {
    const store = await this.load();
    const workspace = store.workspaces.find((candidate) => candidate.id === input.workspaceId);
    if (!workspace) {
      throw new Error("Relay workspace not found.");
  ...
  async revokeDevice(input, relayUrl) {
    const store = await this.load();
    const workspace = store.workspaces.find((candidate) => candidate.id === input.workspaceId);
    if (!workspace) {
      throw new Error("Relay workspace not found.");
    ...
    const remainingApprovedDevices = workspace.devices.filter((candidate) => candidate.id !== input.deviceId && candidate.status === "approved" && candidate.revokedAt === null);
    const expectedRecipientIds = new Set(remainingApprovedDevices.map((candidate) => candidate.id));
    const replacementRecipientIds = new Set(input.replacementWorkspaceKeyWraps.map((wrap) => wrap.recipientDeviceId));
  ...
  async postEvents(input) {
    ...
    let nextSequence = workspace.events.reduce((max, event) => Math.max(max, event.sequence), 0) + 1;
    for (const event of input.events) {
      ...
      const verified = await verify_relay_signature(author.publicKeys.signingPublicKey, relay_event_signature_payload(event), event.signature);
      if (!verified) {
  ...
  async postSnapshot(input) {
    ...
    const verified = await verify_relay_signature(author.publicKeys.signingPublicKey, relay_snapshot_signature_payload(snapshot), snapshot.signature);
    if (!verified) {
      throw new Error("Relay snapshot signature verification failed.");
    }
    const existingIndex = workspace.snapshots.findIndex((candidate) => candidate.snapshotId === snapshot.snapshotId);
    if (existingIndex >= 0) {
  ...
  upsertWorkspaceKeyWrap(workspace, wrap) {
    const existingIndex = workspace.workspaceKeyWraps.findIndex((candidate) => candidate.recipientDeviceId === wrap.recipientDeviceId);
    if (existingIndex >= 0) {
  ...
  buildSession(relayId, workspace, currentDeviceId, relayUrl, adminToken) {
    ...
    const wrappedWorkspaceKey = currentDevice.status === "approved" ? workspace.workspaceKeyWraps.find((wrap) => wrap.recipientDeviceId === currentDevice.id) ?? null : null;
    return {
      relayUrl,
      relayId,
      workspace: {
  ...
  requireApprovedDevice(workspace, deviceId) {
    ...
    try {
      ...
      return {
        ...
        workspaces: parsed.workspaces.map((workspace) => ({
          ...workspace,
          workspaceKeyWraps: Array.isArray(workspace.workspaceKeyWraps) ? workspace.workspaceKeyWraps : [],
          events: Array.isArray(workspace.events) ? workspace.events : [],
          snapshots: Array.isArray(workspace.snapshots) ? [...workspace.snapshots].sort((left, right) => right.baseSequence - left.baseSequence) : [],
          devices: Array.isArray(workspace.devices) ? workspace.devices.map((device) => ({
            ...device,
            role: device.role === "master" || device.role === "member" ? device.role : null,
            status: device.status === "pending" || device.status === "approved" || device.status === "revoked" ? device.status : device.revokedAt ? "revoked" : device.role ? "approved" : "pending",
            approvedAt: typeof device.approvedAt === "string" || device.approvedAt === null ? device.approvedAt : device.role ? device.linkedAt : null,
...
function relay_event_signature_payload(event) {
...
function relay_snapshot_signature_payload(snapshot) {
...
async function verify_relay_signature(signingPublicKey, payload, signature) {
  const publicKeyBytes = Buffer.from(signingPublicKey, "base64");
  const signatureBytes = Buffer.from(signature, "base64");
  const key = await crypto.subtle.importKey("raw", publicKeyBytes, {
    name: "Ed25519"
  }, false, ["verify"]);
  return crypto.subtle.verify({
    name: "Ed25519"
  }, key, signatureBytes, new TextEncoder().encode(payload));
}
...
function buildRelayUrl(request) {
...
if (isMainModule()) {
  startHermesRelay().then(({ host, port }) => {
